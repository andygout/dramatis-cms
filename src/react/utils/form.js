...
const appendBlankArrayItemToStateValue = (stateValue, statePath, opts = {}) => {
	const indexOfLastNumberInStatePath = statePath.map((pathItem) => typeof pathItem === 'number').lastIndexOf(true);
...
const handleAppendArrayItemClick = (stateValue, setStateValue, statePath, event) => {
	...
	const stateValueWithAppendedBlankArrayItem = appendBlankArrayItemToStateValue(stateValue, statePath, {
		isGuaranteedAppendage: true
	});
