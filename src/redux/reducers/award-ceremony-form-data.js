...
import {
	RECEIVE_AWARDCEREMONY_NEW_FORM_DATA,
	RECEIVE_AWARDCEREMONY_EDIT_FORM_DATA
...
const awardCeremonyFormData = (state = Map(), action) => {
	...
	switch (action.type) {
		...
		case RECEIVE_AWARDCEREMONY_NEW_FORM_DATA:
			return fromJS({ ...action.payload, action: FORM_ACTIONS.create });

		case RECEIVE_AWARDCEREMONY_EDIT_FORM_DATA:
